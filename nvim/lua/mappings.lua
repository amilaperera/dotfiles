...
vim.keymap.set(
    ...
    ">",
    ">gv",
    { silent = true, remap = false, desc = "Shift a visual selection right (retains selection)" }
)
vim.keymap.set(
    "v",
    "<",
    "<gv",
    { silent = true, remap = false, desc = "Shift a visual selection left (retains selection)" }
)
...
vim.keymap.set("n", "yox", function()
    toggle_options("cursorline", "cursorcolumn")
end, { silent = true, desc = "Toggle both cursoline and cursorcolumn" })
...
end, { silent = true, desc = "Toggle colorcolumn=+1 or none" })

-- prev/next tab with a count
vim.keymap.set("n", "[t", function()
    vim.fn.execute("normal " .. vim.v.count1 .. "gT")
end, { silent = true, desc = "Go to the [count] previous tab" })

vim.keymap.set("n", "]t", function()
    for i = 1, vim.v.count1 do
        vim.fn.execute("normal gt")
    end
end, { silent = true, desc = "Go to the [count] next tab" })
...
end, { silent = true, desc = "Insert empty [count] line(s) before the current line" })

vim.keymap.set("n", "]<Space>", function()
    insert_blanks(vim.v.count1, true)
    vim.fn.execute("normal k")
end, { silent = true, desc = "Insert empty [count] line(s) after the current line" })
...
vim.keymap.set("n", "yoz", function()
    toggle_window_zoom()
end, { silent = false, desc = "Toggle window zooming" })
...
end, { expr = true, desc = "Use grep in a native way in the current directory" })
